...
if (in_array($a, array_keys($settings['xhr'])) && in_array($m, $settings['xhr'][$a])) {
    ...
    /* PROFILES */
        ...
        /* Save a profile */
            ...
            if (strlen($_POST['pseudo']) > 12)
                $errors[] = 'Your nickname is too long, shouldbe no more than 12 symbols.';
    ...
    /* Everything CHAT related start here */
        ...
        switch ($m) {
            ...
            case 'fetch':
                $output = [];
                $output['msgs'] = $db->getRows("SELECT * FROM %s WHERE `receiver`='%s' ORDER BY `time` ASC", $db->table('messages'), 'default');
                jD('msgs', $output['msgs']);
            break;
        }
    }
